class RomanNumerals:
    ...
    def check_roman_repeat_rules(roman_string: list) -> bool:
        ...
        MAX_REPEAT_COUNT_X = (
            4  # this can only be repeated for 4 times while it's 'XXXIX'
        )
        MAX_REPEAT_COUNT_C = (
            4  # this can only be repeated for 4 times while it's 'CCCXC'
        )
        MAX_REPEAT_COUNT_M = (
            4  # this can only be repeated for 4 times while it's 'MMMCM'
        )
        ...
        if roman_string.count("I") > MAX_REPEAT_COUNT_I:
            return False
        if roman_string.count("X") > MAX_REPEAT_COUNT_X or (
            roman_string.count("X") == 4 and "XXXIX" not in roman_string
        ):
            return False
        if roman_string.count("C") > MAX_REPEAT_COUNT_C or (
            roman_string.count("C") == 4 and "CCCXC" not in roman_string
        ):
            return False
        if roman_string.count("M") > MAX_REPEAT_COUNT_M or (
            roman_string.count("M") == 4 and "MMMCM" not in roman_string
        ):
            return False
    ...
    def check_roman_subtract_rules(roman_string: list) -> bool:
        ...
        while i < len(roman_string):
            ...
            if (i < len(roman_string) - 1) and RomanNumerals.CURRENCY[
                roman_string[i]
                ...
                if (
                    last_subtract_roman != 0
                    and current_arabic_value + last_arabic_value >= last_subtract_roman
                ):
                    return False
                else:
    ...
    def check_roman(roman_string: list) -> bool:
        return (
            RomanNumerals.check_roman_basic_symbols(roman_string)
